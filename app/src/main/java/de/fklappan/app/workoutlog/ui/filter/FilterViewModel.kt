...
class FilterViewModel(private val useCaseFactory: UseCasesFactory,
                      private val schedulerIo: Scheduler,
                      private val schedulerMainThread: Scheduler,
                      private val modelMapper: GuiModelMapper) :
    RxViewModel() {

    private val _tagChipLiveData = MutableLiveData<String>()
    // expose read only workoutState
    val tagChipLiveData: LiveData<String>
        get() = _tagChipLiveData

    public fun onSearchTextChanged(text: String) {

        for (text in text.trimStart().split(" ")) {
